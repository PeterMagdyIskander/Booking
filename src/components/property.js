...
today = yyyy+'-'+mm+'-'+dd;
  ...
  const [place, setPlace] = useState(
    props.prop.sections[0].sectionName
  );
  const [hall, setHall] = useState(
    props.prop.halls[0].hallName
  );
    ...
    <div>
      ...
      <select value={place} onChange={handlePlaceChange}>
        {props.prop.sections.map((section) => {
          return <option key={section.sectionName} value={section.sectionName}>{section.sectionName} </option>;
        })}
      </select>
        <Rooms sectionRooms={props.prop.sections[getIndex(place,props.prop,true)].sectionsRooms}/>

      <select value={hall} onChange={handleHallChange}>
        {props.prop.halls.map((hall) => {
          return <option key={hall.hallName} value={hall.hallName}>{hall.hallName} </option>;
        })}
      </select>
      <p>capacity {props.prop.halls[getIndex(hall,props.prop,false)].hallCapacity}</p>
      <h1>{props.Location}</h1>
      <h1>{props.website}</h1>
      <button disabled={authed} onClick={openModal}>book now</button>
